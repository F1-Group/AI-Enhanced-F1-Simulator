def build_user_prompt(telemetry, coaching_request, knowledge=""):
    ...
    # Knowledge section
    knowledge_section = (
        "GENERAL BACKGROUND CONCEPT (not evidence about this specific lap) "
        "any corner, chicane, or number mentioned here is just an illustrative "
        "example from the knowledge base, not something that happened in this "
        f"drive):\n{knowledge}\n"
    ) if knowledge else ""

    return f"""
TELEMETRY DATA (Do not invent a number for any of these fields unless that same
