...
def process_single_error(error, system_prompt, audio_manager, force_fallback=False):
    ...
    print(f"[AI Thread] Analyzing Telemetry Infraction: {error_type} at {error.get('corner', '?')}")

    # Ensure the telemetry variables are correctly extracted at the outermost layer.
    telemetry = error.get("telemetry") or error.get("telemetry_snapshot")
    if not telemetry:
        telemetry = {"timestamp": error.get("timestamp", 0.0), "lap_distance": 1500.0, "speed_kmh": 185.5}

    # If it is a precise numerical hint generated by the fast-layer real-time snapshot, 
    # pass it through directly to bypass the LLM.
    if error.get("is_realtime") and error.get("coaching_hint"):
        answer_text = error["coaching_hint"]
        print(f"[AI Thread] Direct Pass-through for Realtime Telemetry Hint: {answer_text}")
        
    # Only enable the fallback script for slow-layer review when disconnected.
    elif force_fallback or not _RUNTIME_LLM_OK:
        answer_text = get_fallback_text(error_type)
        print(f"[AI Worker Fallback] Using professional rule-based script for {error_type}")
        
    # Normal execution branch for the slow-layer LLM
    else:
    ...
    if not result or not result.get("is_valid", False):
        raise ValueError(f"Guardrail violation at: {error.get('corner')}")

    # Ensure that when the fast layer passes the guardrails, 
    # the returned feedback contains the dynamic data calculated by the fast layer.
    if error.get("is_realtime") and error.get("coaching_hint"):
        result["feedback"] = error["coaching_hint"]
