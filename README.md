...
The middleware never calls a remote LLM API. All coaching text is generated by **IBM Granite 2B** (`granite3-dense:2b`) running entirely offline through a local **Ollama** daemon — there is no cloud endpoint, API key, or network egress involved in inference.

### 3.1 Connection Architecture

`llm/llm_client.py` owns the connection. It talks to Ollama through the official `ollama` Python package rather than raw HTTP, so the client only needs a model name — Ollama's own daemon handles the actual socket on its default address (`http://localhost:11434`):
...
At startup, `init_granite_model()`:
1. Checks whether the local Ollama service is already running (`_client.list()`).
2. If not, spawns `ollama serve` as a background process (OS-specific launch logic for Windows / macOS / Linux).
3. Sends a throwaway `"hi"` chat request so the model weights are loaded into memory before the race starts, avoiding a cold-start delay on the first real coaching call.

`get_ai_link_status()` is a lightweight health check (`_client.chat(...)` with a `"ping"` message) used by the Dashboard GUI to report whether Granite is reachable, and `ask_race_engineer(system_prompt, user_prompt)` is the single call site used for every actual coaching request — it retries once on failure, then raises, letting the caller (`ai_core.py`) decide whether to fall back to the static `FALLBACK_SCRIPTS` text or trip the circuit breaker.

### 3.2 Configuration (`llm_client.py`)

There is currently no separate `config.py` — because inference is entirely local and unauthenticated, the only settings that exist are two module-level constants in `llm/llm_client.py`:
...
No API key field exists because Ollama serves models locally with no authentication by default.

To point the system at a different model, or at an Ollama instance running on a non-default host/port (e.g. a separate machine on the LAN), edit these two lines directly:
...
After changing `MODEL_NAME`, pull the new model first (`ollama pull granite3-dense:8b`) — `init_granite_model()` will warn but not fail if the model hasn't been downloaded yet.

### 3.3 Prompt Structure (`prompts.py`)

Each coaching call sends two messages to Granite, assembled from two different sources:
...
* **System prompt** — `coaching_style.get_system_prompt(style)` selects one of three fixed personas (`aggressive`, `supportive`, `technical`; default `technical`). Every persona shares the same `_SHARED_RULES` block, which encodes the domain knowledge Granite is allowed to reason with (tyre slip thresholds, fuel-weight effect, sector loss thresholds, etc.) and hard constraints (never invent a number that isn't in the telemetry, never say "delta", reply like a real race engineer on the radio).
* **User prompt** — `prompts.build_user_prompt(telemetry, coaching_request, knowledge)` formats the live telemetry snapshot into plain-text key/value lines Granite can read directly, e.g.:
...
`telemetry` is the raw dict captured off the UDP stream (see [Section 2.3](#23-telemetry-integration-architecture)); `coaching_request` is the hint text generated by the error-detection layer (e.g. `"Brake 25m earlier before T1."`); `knowledge` is an optional block of retrieved chunks from the RAG store (`llm/rag.py`), explicitly labelled as *general background* rather than evidence about the current lap, so Granite can't mistake a knowledge-base example for something that happened on this drive. The one-sentence / 20-word limit keeps responses short enough to speak aloud without lagging behind the live telemetry stream.
